...
interface TravelResult {
  text: string;
  image?: string; // Added to support inspirational images
...
const TravelAgentUI: React.FC<TravelAgentUIProps> = ({ onTaskComplete }) => {
  ...
  const tabs: { id: Tab; icon: React.FC<any>; label: string }[] = [
    ...
    { id: 'places', icon: Search, label: currentText.tasks.findPlacesOfInterest },
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`p-4 h-full flex flex-col`} style={{ background: theme.colors.background }}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {activeTab === 'places' && (
            ...
            <button onClick={() => executeTask('findPlacesOfInterest', { location: placesLocation, placeType: placesType })} disabled={isLoading || !placesLocation || !placesType} className={buttonClass}>{isLoading ? globalText.loading : currentText.tasks.findPlacesOfInterest}</button>
            </div>
        )}
        ...
        {result && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4 p-4 rounded-lg bg-surface">
            {activeTab === 'itinerary' && result.image && (
              <img src={result.image} alt="Inspirational trip visual" className="w-full rounded-lg mb-4 object-cover" />
            )}
